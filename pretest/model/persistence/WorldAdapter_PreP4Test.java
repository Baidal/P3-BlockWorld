...
public class WorldAdapter_PreP4Test {
	...
	public void testGetMapBlock1() {
		...
		/**
		 * Comprobamos que el numero total de elementos es 16^3
		 */
		assertEquals(mapBlock.size(),16*16*16);
		...
		for (Location relative : setLoc) {
			...
			try {
				...
				if(block == null) //SI EL MUNDO DEVUELVE NULL, EN ESA POSICIÓN mapBlock TENDRÁ UN BLOQUE DE AIRE
					block = BlockFactory.createBlock(Material.AIR);
				
				Block aux = mapBlock.get(relative);
				assertEquals(aux,block);
			} catch (Exception e) {
	...
	public void testGetMapBlock2() {
		...
		for (Location relative : setLoc) {
			...
			try {
				...
				if(block == null && Location.check(absolute)) //SI EL MUNDO DEVUELVE NULL, EN ESA POSICIÓN mapBlock TENDRÁ UN BLOQUE DE AIRE
					block = BlockFactory.createBlock(Material.AIR);
	...
	public void testGetCreatures1() {
		...
		for(int i=0;i<listCreature.size();i++) {
			...
			if(listCreature.get(i).getSymbol()=='M'&&okLocation(posCreature, base))
				nMonsters++;
			else if(listCreature.get(i).getSymbol()=='L'&&okLocation(posCreature, base))
				nAnimals++;
		}
		assertEquals(3,nAnimals,0.01);
		assertEquals(12,nMonsters,0.01);
	...
	public void testGetCreatures2() {
		...
		assertEquals(4,nAnimals,0.01);
		assertEquals(3,nMonsters,0.01);
...
public void testGetItems1() {
		...
		int numitems=0;
		for (Location pos : setLoc) {
			  if(okLocation(pos,base)&&mapItems.containsKey(pos)) {
				  numitems++;
			  }
			}
		assertEquals(24,numitems,0.01);
...
public void testGetItems2() {
		...
		assertEquals(setLoc.size(),mapItems.size(),0.01);
		assertEquals(8,numitems,0.01);
